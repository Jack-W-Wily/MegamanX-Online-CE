...
public class HexaInvoluteStateWC : CharState {
	HexaInvoluteProjWC proj;
	bool startGrounded;
	float ammoTime;
	...
	public override void update() {
		...
		if (!once && character.frameIndex >= 2) {
			once = true;
			proj = new HexaInvoluteProjWC(new HexaInvoluteWeaponWC(), character.getFirstPOIOrDefault(), 1, player, player.getNextActorNetId(), rpc: true);
		}
...
public class HexaInvolutePartWC {
	...
	public HexaInvolutePartWC(Point pos) {
		time = 0;
...
public class HexaInvoluteProjWC : Projectile {
		...
		for (int i = 0; i < 6; i++) {
			...
			if (partCooldown == 0) {
				if (!Options.main.lowQualityParticles()) {
					parts.Add(new HexaInvolutePartWC(dest));
				}
			}
		}
		if (partCooldown == 0) {
			if (!Options.main.lowQualityParticles()) {
				parts.Add(new HexaInvolutePartWC(pos));
			}
		}
