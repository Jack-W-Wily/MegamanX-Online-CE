...
public class XKai : MegamanX {
	...
	public XKai(
		...
		spriteFrameToSounds["mmx_run/4"] = "run";
		spriteFrameToSounds["mmx_run/9"] = "run";
		specialButtonMode = 0;	
		charId = CharIds.XKai;
	...
	public override bool isToughGuyHyperMode() {
		return player.XKaiUnlockBarrier && OverDrive;
	}
	...
	public override bool attackCtrl() {
		...
		&& charState is not RMXDoubleKick) {
			...
			if (player.input.isHeld(Control.Down, player)) {
					...
					if (player.input.isL2Held(player) && player.XKaiParry) {
						enterParry();
					}else {
