...
public class RideChaser : Actor, IDamagable {
	...
	public override Projectile getProjFromHitbox(Collider hitbox, Point centerPoint) {
		if (player != null && canRunEnemyOver()) {
			return new GenericMeleeProj(hitWeapon, centerPoint, ProjIds.RideChaserHit, player, 4, Global.defFlinch, addToLevel : true);
		} else if (player != null && canHurtEnemy()) {
			return new GenericMeleeProj(hitWeapon, centerPoint, ProjIds.RideChaserHit, player, 2, 0, addToLevel : true);
		}
		return null;
	}
