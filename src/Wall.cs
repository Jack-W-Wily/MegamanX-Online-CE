...
public class CrackedWall : Actor, IDamagable {
	...
	public static float? canDamageCrackedWall(int projId, CrackedWall cw) {
		...
		if (projId == (int)ProjIds.MechPunch || projId == (int)ProjIds.MechKangarooPunch || projId == (int)ProjIds.MechGoliathPunch || projId == (int)ProjIds.MechDevilBearPunch) return 4;
		...
		if (projId == (int)ProjIds.Torpedo) return 4;
		if (projId == (int)ProjIds.TorpedoCharged) return 4;
		...
		if (projId == (int)ProjIds.TornadoFang) return 1;
		if (projId == (int)ProjIds.TornadoFang2) return 2;
		if (projId == (int)ProjIds.TornadoFangCharged) return null;
		if (projId == (int)ProjIds.TriadThunderQuake) return 20;
		if (projId == (int)ProjIds.Headbutt && cw?.flag == 1) return 12;
		if (projId == (int)ProjIds.VileMissile) return 4;
		if (projId == (int)ProjIds.PopcornDemon) return 2;
		if (projId == (int)ProjIds.PopcornDemonSplit) return 3;
		if (projId == (int)ProjIds.LaunchOMissle) return null;
		if (projId == (int)ProjIds.LaunchOTorpedo) return null;
		if (projId == (int)ProjIds.NecroBurst) return 22;
		if (projId == (int)ProjIds.SparkMPunch) return 22;
		...
		if (projId == (int)ProjIds.WheelGSpinWheel) return 5;
		...
		if (projId == (int)ProjIds.TunnelRDash) return 12;
