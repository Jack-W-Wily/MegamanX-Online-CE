...
public class StrikeChain : Weapon {
	...
	public override void shoot(Character character, int[] args) {
		...
		if (chargeLevel == 2) {
			...
			new StrikeChainSemiCharged(this, pos, xDir, player, player.getNextActorNetId(), upOrDown, true);
		}
	}
}
...
public class StrikeChainSemiCharged : Projectile {
	...
	public static Projectile rpcInvoke(ProjParameters arg) {
		return new StrikeChainSemiCharged(
			StrikeChain.netWeapon, arg.pos, arg.xDir, arg.player, arg.netId
		);
	}
...
public class StrikeChainLightningState : CharState {
	int state;
	Point poi;
	public StrikeChainLightningState() : base("gigacrush", "") {
		superArmor = true;
	}
	...
	public override void update() {
		...
		if (state == 0 && character.getFirstPOI() != null) {
			state++;
			poi = character.getFirstPOIOrDefault();
			new WolfSigmaBeam(player.weapon, poi.addxy(0, -130), character.xDir, 1, 1, player, player.getNextActorNetId(), rpc: true);
		}

		if (state == 1 && character.frameIndex == 5) {
			state++;
			var spawnPos = poi.addxy(50 * character.xDir, -120);
			...
			new WolfSigmaBeam(player.weapon, spawnPos, character.xDir, 1, 2, player, player.getNextActorNetId(), rpc: true);
		}

		if (state == 2 && character.frameIndex == 8) {
			state++;
			var spawnPos = poi.addxy(-50 * character.xDir, -120);
			var closestTarget = Global.level.getClosestTarget(poi, player.alliance, true, aMaxDist: 150);
			if (closestTarget != null) spawnPos.x = closestTarget.pos.x;
