...
public class VileClassic : Character {
	...
	public override bool attackCtrl() {
		...
		if (grounded || charState is AirDash){
		changeState(new StunShotAttack(grounded), true);
		}
		if (!grounded && charState is not AirDash){
			changeState(new ExplosiveRoundVC(), true);
		}
		gizmoCooldown = 0.5f;
		}
		if (WeaponRightHeld) {
	...
	public bool RideArmorAttacks() {
		...
		if (rideArmor != null && raState != null && !raState.isHiding) {
			if (raStates) {
				...
				if (stunShotPressed && !HeldDown && gizmoCooldown == 0) {
					new StunShotProj(new VileMissile(VileMissileType.ElectricShock)
					, pos.addxy(15 * xDir, -10), getShootXDir(),
					 0, player, player.getNextActorNetId(),
					  getVileShootVel(true), rpc: true);	
					  gizmoCooldown = 0.5f;
				}
