...
public class Buster : Weapon {
	...
	public Buster() : base() {
		...
		weaponSlotIndex = 0;
		shootSounds = new string[] { "buster", "buster", "buster", "buster", "buster" };
		rateOfFire = 0.15f;
		canHealAmmo = false;
		drawAmmo = false;
		drawCooldown = false;
	...
	/*public Buster() : base() {
		...
		killFeedIndex = 0;
		weaponBarBaseIndex = 0;
		shootSounds = new List<string>() { "buster", "buster2", "buster3", "buster3" };
	...
	//	bool isHyperX = ((player.character as MegamanX)?.isHyperX == true);
		...
		if (player.ownedByLocalPlayer)player.character.playSound("buster3", sendRpc: true);
		}
		if (!player.hasUltimateArmor() && player.HasFullForce() && chargeLevel >= 3) {
			...
			if (player.ownedByLocalPlayer) 	player.character.playSound("buster2", sendRpc: true);
		}
		if (!player.hasUltimateArmor() && player.HasFullForce() && (player.character as MegamanX).unpoShotCount > 0) {
			if (player.ownedByLocalPlayer) new BusterUnpoProj(this, pos, xDir, player, netProjId);
			...
			if (player.hasUltimateArmor()) {
					...
					new BusterPlasmaProj(this, pos, xDir, player, netProjId);
						player.character.playSound("plasmaShot", sendRpc: true);	
...
public class BusterProj : Projectile {
	public BusterProj(
		...
		if (player.HasFullShadow()){
		vel = new Point(200 * xDir, -10 * Helpers.randomRange(0,20));
		}
		if (rpc) {
...
public class Buster2Proj : Projectile {
	public Buster2Proj(Weapon weapon, Point pos, int xDir, Player player, ushort netProjId) : base(weapon, pos, xDir, 350, 2, player, "buster2", 4, 0, netProjId, player.ownedByLocalPlayer) {
...
// Down here is where the Cross Shot actually happens
	public override void onCollision(CollideData other) {
		base.onCollision(other);	
				...
				if (other.gameObject is BusterX3Proj1 X3shot && X3shot.ownedByLocalPlayer && !destroyed) {
					...
					new Buster3Proj(weapon, pos, xDir, 10, owner, owner.getNextActorNetId(), rpc: true);
					} else {
	...
	public override void update() {
		base.update();
		if (type == 9 || type == 0 && owner.HasFullMax()) {
...
public class Buster4Proj : Projectile {
	...
	public Buster4Proj(
		...
		Global.defFlinch, 0.05f, netProjId, player.ownedByLocalPlayer
	) {
