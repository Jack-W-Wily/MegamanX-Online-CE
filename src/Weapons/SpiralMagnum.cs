...
public class SpiralMagnumProj : Projectile {
	...
	public override void update() {
		...
		if (victim != null) {
			...
			bool canBlock = reflectable && !weakness && hitChar != null && hitChar.player.isZero && (hitChar.sprite.name == "superzero_block" || (hitChar.sprite.name.Contains("superzero_attack") && hitChar.sprite.getCurrentFrame().hitboxes.Count > 0));
			if (canBlock && hitChar.sprite.name != "superzero_attack_air2") {
